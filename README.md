...
* First, we will do the initial collateral TX and send exactly needed amount of MLDN due to current MN collateral to one of our addresses. To keep things sorted in case we setup more masternodes we will label the addresses we use.

  - Open your MLDN wallet and switch to the "Receive" tab.
  ...
  - Once all addresses are created send needed amount of MLDN due to current MN collateral each to them. Ensure that you send exactly needed amount of MLDN due to current MN collateral and do it in a single transaction. You can double check where the coins are coming from by checking it via coin control usually, that's not an issue.
...
Close your wallet and open the Corium Appdata folder. Its location depends on your OS.

* **Windows:** Press Windows+R and write %appdata% - there, open the folder Corium.
* **macOS:** Press Command+Space to open Spotlight, write ~/Library/Application Support/Corium and press Enter.
* **Linux:** Open ~/.corium/

In your appdata folder, open masternode.conf with a text editor and add a new line in this format to the bottom of the file:

```bash
masternodename ipaddress:17999 genkey collateralTxID outputID
```

An example would be

```
mn1 127.0.0.2:17999 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
```
...
```bash
bash <( curl https://raw.githubusercontent.com/coriumplatform/Corium-MN-Install/master/install.sh )
```
...
**PLEASE NOTE**: Do not choose the advanced installation option unless you have experience with Linux and know what you are doing - if you do and something goes wrong, the Corium team CANNOT help you, and you will have to restart the installation.
...
```
bash <( curl https://raw.githubusercontent.com/coriumplatform/Corium-MN-Install/master/refresh_node.sh )
```
...
```
bash <( curl https://raw.githubusercontent.com/coriumplatform/Corium-MN-Install/master/update_node.sh )
```
...
If you want to make the installation process fully non-interactive, you need to provide Corium with arguments for the mode to use, the external IP, private key, and wether to use fail2ban, UFW and the bootstrap, and then also add the `--no-interaction` parameter. Please not that this will not tell you to activate your masternode from your wallet after the node has finished syncing, so it will not run until you do.
